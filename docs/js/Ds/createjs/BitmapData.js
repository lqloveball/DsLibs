 ...
 * @classdesc:像素处理高级类，这个类来至kudox http://kudox.jp/。我只是在巨人基础上进行一些修改更新
 * @extends
 * @example: 举例
 * @author:
...
_stage.addChild(_shape);</code></pre>
	**/
    ...
    /**
     * 改写一个叠加滤镜处理的方法 by cagoeAG.Maksim
     * @param  {[type]} source [原图片对象]
     * @param  {[type]} sourceRect   [绘制区域]
     * @param  {[type]} destPoint    [绘制启点]
     * @param  {[type]} blendMode    [叠加模式]
     * @return {[type]}              [description]
     */
    p.copyCanvas = function(source, sourceRect, destPoint, blendMode) {

        //如果想要用HTML对象的话
        /*
        var drawingCanvas = document.createElement("canvas");
        var drawingContext = this.drawingCanvas.getContext("2d");
        drawingCanvas.setAttribute('width', sourceCanvas.width);
        drawingCanvas.setAttribute('height', sourceCanvas.height);

        	drawingContext.drawImage(sourceCanvas,
        	0, 0, sourceCanvas.width, sourceCanvas.height,
        	0, 0, sourceCanvas.width, sourceCanvas.height);

        	sourceCanvas=drawingContext;
        */
        ...
        if (source instanceof BitmapData || source instanceof createjs.Stage) {
            ...
            if (!!source.cacheCanvas) {
                sourceCanvas = source.cacheCanvas;
            } else {
                throw new Error("The source of BitmapData.copyCanvas(), must be called DisplayObject.cache().");
            }
        }
        ...
        var BlendMode = createjs.BlendMode;
...
_bmd02.fillRect(rect, color02);</code></pre>
  ...
  _stage.update();
	...
	**/
    ...
    p.getColorBoundsRect2 = function(color, findColor) {
