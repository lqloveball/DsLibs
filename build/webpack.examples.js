...
const common = require('./webpack.examples.base.js');
 ...
 * @param epath
 */
function dealExamplesEntry(entry,epath){
    let source='./docs/examples/'+epath+'/src/';
    let _source = resolveBowerPath(source);
    let _files = glob.sync(_source + '/*.js');
    let _outPath='../examples/'+epath+'/js/';
    // console.log('files:', _files);
    _files.forEach(function (file) {
        console.log('file:',file);
        let _name = file.slice(file.lastIndexOf('/') + 1, file.lastIndexOf('.'));
        console.log('name:',_name);
        let _outputName = path.join(_outPath, _name);
        // console.log('outputName:',_outputName);
        let _file = path.resolve(__dirname, '..', file);
        // console.log('绝对 file:',_file);
        entry[_outputName] = _file;
...
entry['./exs/edslibs/createjsFrameWork'] = [
    'createjs',
    'dscreatejs',
];

//框架开发编译
dealEntry(entry, './H5EasyTemplate/source/eds', './exs/edslibs/');

//案例入口文件
dealExamplesEntry(entry,'test0');
