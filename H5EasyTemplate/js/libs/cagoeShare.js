...
!(function () {
     ...
     */
    function setWXApiConfig(url) {
        if (WXApiConfigUrl !== "") return;
        WXApiConfigUrl = url;
        //不是绝对路径
        url = getAbsoluteUrl(url);
        $.ajax({
            url: url + (url.indexOf("?") > 0 ? "&" : "?") + "url=" + encodeURIComponent(location.href),
            type: 'get',
            dataType: "jsonp",
            success: function (data) {
                console.log('data');
                eval("(" + data + ")");
                //如果对接别人第三方客户的分享接口，原生wx.config设置如果下
                /*
                var sign = data.data;
                wx.config({
                    debug: false,
    ...
    wx.ready(function () {bindJsApiEvent();});
