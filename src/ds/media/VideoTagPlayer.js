...
class VideoTagPlayer extends EventDispatcher {
    ...
    constructor(url, opts) {
        ...
        if (url !== undefined && url !== null) {
            ...
            if (opts.inSiteLoad!==undefined&&opts.inSiteLoad===true) {
                $(this._video).attr('preload', "auto");
                this._load(url)
            } else {
                _video.src = url;
            }
    ...
    _load(url) {
        console.log('video 标签提前加载：',url);
        let _self=this;
        let _video = this._video;
        let _xmlrequest = new XMLHttpRequest;
        _xmlrequest.open("GET", url, true);
        _xmlrequest.responseType = "blob";
        _xmlrequest.onload = function () {
            if (200 === this.status && "video/mp4" === this.response.type) {
                let _response = this.response;
                // console.log('_response:',_response);
                let _dataUrl;
                if(window.URL)_dataUrl=  window.URL.createObjectURL(_response);
                else _dataUrl=  window.webkitURL.createObjectURL(_response);
        ...
        _xmlrequest.onerror = function (e) {
            console.log(e);
        };

        _xmlrequest.onprogress=function (e) {
            let _progress = e.loaded / e.total;
            // console.log('_progress:',_progress);
            _self.ds({type: 'progress', progress: _progress});
        };
        _xmlrequest.send();
    }
    ...
    /**
     * 是否执行开始加载 这个不同与其他播放器默认是可以算开始加载的了
     * @return {boolean}
     */
    get isStarload() {
