...
class ScrollPanel extends EventDispatcher {
    ...
    constructor(display, contentHeight, width, height, opts) {
        ...
        this._alloyTouch = new AlloyTouch({
            ...
            vertical: opts.vertical !== undefined ? opts.vertical : true,
            target: _target, //运动的对象
            property: "y",  //被运动的属性
            min: this._scroll, //不必需,运动属性的最小值
            max: 0, //不必需,滚动属性的最大值
            sensitivity: 1,//不必需,触摸区域的灵敏度，默认值为1，可以为负数
            factor: opts.factor !== undefined ? opts.factor : 1,//不必需,表示触摸位移与被运动属性映射关系，默认值是1
            moveFactor:opts.moveFactor !== undefined ? opts.moveFactor : 1,//不必需,表示touchmove位移与被运动属性映射关系，默认值是1
            outFactor:opts.outFactor !== undefined ? opts.outFactor : 0.3,//不必需,表示touchmove位移与被运动属性映射关系，默认值是1
