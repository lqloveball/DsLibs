 ...
 *  事件 progress加载进度   complete 加载完成  canplaythrough可以缓冲播放  onplayend播放完成  Play播放  Pause暂停
 ...
 * @example://默认初始化的时候 视频不放到可见区域
   _VideoInteractivePlayer = new Ds.media.VideoPlayerByFrames({
        imagesList:_ImagesList,//如果提前传入图片队列，那就会忽略 imagesPath imagePrefix imgType totalframes参数 因为提前传入队列 所有没 complete事件的发出，但有onload这个参数的方法执行
...
(function(factory) {
    ...
    function VideoPlayerByFrames(data) {
        ...
        var OnPlayEnd = data.onplayend ? data.onplayend : null;
        var Progress = data.onprogress ? data.onprogress : null;
        var Complete = data.oncomplete ? data.oncomplete : null;
        ...
        if (audio) {
            ...
            //audio.addEventListener('canplaythrough', _self.audioPlayEvent); //pause()触发
        ...
        function loadImages() {
            ...
            imagesLoadNum += 1;
            if (Progress) Progress({"progress":imagesLoadNum / imagesList.length});
            //console.log(imagesLoadNum / imagesList.length);
            _self.ds({
                type: 'progress',
