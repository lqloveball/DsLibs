(function(factory) {
  ...
  function ThroughSpace(three3DModelOpts, sliderOpts) {
    ...
    this.StatrLoad = function() {
      console.log('_TextureLoadList:', _TextureLoadList.length);
      //开始加载纹理资源，并且创建纹理字典
      _TextureDc = _LoadManager.LoadTextureList(_TextureLoadList, function() {
        console.log('_TextureLoadList loadEnd');
    ...
    /**
         ...
         * @constructor
         */
    function TextureFileLoadFun(data) {
    ...
    this.CreateSpace = function() {
      if (_CreateSpaceBool)
        return;
      console.log('ThroughSpace CreateSpace');
      _CreateSpaceBool = true;
      //开始创建场景组队列
      CreateGroupList();
    };
    /**
    ...
    function CreateGroupList() {
      ...
      //创建滑块控制器
      if (sliderOpts)
        _Self.CreateSlider(sliderOpts);

      //穿梭空间构建完成
      _Self.ds({type: 'createEnd'});
    }
    ...
    this.Greate3DObject = function(data, map) {
      ...
      if (!data.type) {
        ...
        if (map) {
          ...
          _Self.on('fileLoad', function(e) {
            var _data = e.data;
            if (_data.src === data.src) {
              if (_TextureDc[data.src] && _TextureDc[data.src].texture) {
                map = _TextureDc[data.src].texture;
                _width = map.image.width;
                _height = map.image.height;
                _material.map=map;
                data.width = _width * _scale;
    ...
    function UpDateScene() {
      ...
      for (var i = 0; i < _SceneList.length; i++) {
        ...
        if (_Value >= _groupData.start - _BgFar && _Value <= _groupData.end) {
          ...
          _SceneGroup.add(_scene);
          if (_groupData.bg)
            _BGGroup.add(_groupData.bg.obj);
          ...
          if (_groupData.bg)
            _BGGroup.remove(_groupData.bg.obj);
          }
        }
    ...
    function UpDateChild(childData) {
      ...
      if ((_obj && _Effects) || (_obj && _childData.effects)) {
        ...
        if (!_childData.effects && _Effects) {
          //进场滤镜
          if (_obj.material)
            _obj.material.opacity = _opacity * _speed;
          if (_noOutEffect) {
            //退场效果
            if (_obj.material && _hasOut)
              _obj.material.opacity = _opacity * _outSpeed;
            }
