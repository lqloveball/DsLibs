(function (factory) {
  ...
  function ThroughSpace(three3DModelOpts, sliderOpts) {
      ...
      //纹理索引
      var _TextureDc;
      Object.defineProperty(this, "TextureDc", {get: function() {return _TextureDc;},});
      //纹理加载队列
          ...
          for (var i = 0; i < _SceneList.length; i++) {
              ...
              if (_Value >= _groupData.start - _BgFar && _Value <= _groupData.end) {
                  ...
                  for (var j = 0; j < _groupData.list.length; j++) {
                      ...
                      var _pt,_z,_effectSpeed,_opacity,_speed;
                      if((_obj && _Effects) || (_obj && _childData.effects)||_childData.upDate){
                        ...
                        //计算用1000做渐变动画
                        _speed =Math.min( Math.max(_z / _effectSpeed, 0),1);
                      }
                      //自动滤镜
                      ...
                      if (_childData.upDate){
                         _childData.upDate({z:_Value,bgz:_z,speed:_speed,pt:_pt});
                      }
                  }
