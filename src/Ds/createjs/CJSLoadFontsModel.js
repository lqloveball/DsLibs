 ...
 * @extends
 * @example:
 * var _FontsModel=new Ds.createjs.CJSLoadFontsModel('./fonts/HWXHOTxt/');
    _FontsModel.on('loadFontsComplete',loadFontsComplete)
    _FontsModel.LoadUserWordFonts('测试下我要的字体ABC12!@#a');
    function loadFontsComplete(){
        log('loadFontsComplete');
        log(_FontsModel.GetFontData('我'));
        log(_FontsModel.GetFontPathData('我'));
        var font=_FontsModel.GetShapeFont('我','#000000',15);
        _Root.addChild(font);
        // shape.scaleY=-1;
        // shape.y=1000;
        log(font.y)
...
window.Ds.createjs.CJSLoadFontsModel=CJSLoadFontsModel;
/**
 * 动态字体类
