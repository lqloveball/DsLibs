(function (factory) {
    ...
    InputInteractive.ProvincesCityLinkComponents = function (pUrl, cUrl, domBox, opts) {
        ...
        //事件发送者
        var _eventer =new Ds.EventDispatcher();
        _provinceDom._eventer=_eventer;
        _cityDom._eventer=_eventer;
        ...
        var _provinceConfig = {
            ...
            callback: function () {
                ...
                _provinceDom._eventer.ds({
                    type:'provinceUpdate'
                });
            },
            select: function (option, index, data) {
                provinceSelectUpCity(option, index, data);
                _provinceDom._eventer.ds({
                    type:'provinceSelect'
                });
            }
    ...
    InputInteractive.SetShowLabelUpChange = function (selectDom, showLabelDom, opts) {
        ...
        function changeFun() {
            InputInteractive.UpSelectShowLabel(selectDom, showLabelDom, opts);
        }
    ...
    /**
     * 根据参数值进行设置选择项的selectedIndex
     * 一般常用在数据回流填充；
     * @param selectDom
     * @param value      匹配的数据
     * @param valueName   配置数据来至Object对象值名时候需要设置，不设置进行_option.data 或者 option.value 匹配对比
     * @constructor
     */
    InputInteractive.SetSelectOptionByValue=function (selectDom,value,valueName) {
        selectDom = getHTMLElement(selectDom);
        var _options=selectDom.options;
        var _index=selectDom.selectedIndex;
        var _option;
        for (var i = 0; i < _options.length; i++) {
            _option = _options[i];
            var _data=_option.data;
            if(valueName&&typeof(_data)==='object'&&_data[valueName]){
                // console.log('------1');
                if(_data[valueName]===value)_index=i;
            }
            else if(_data){
        ...
        if(_option){
            ...
            if(selectDom.changeFun)selectDom.changeFun();
            if(selectDom.selectOptionFun)selectDom.selectOptionFun();
        }
    };
    /**
     * $对象转换成HTMLElement对象
