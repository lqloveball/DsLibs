...
(function (factory) {
    ...
    function SliderDistance(sliderObject, valueName, opts) {
        ...
        var _Interval = opts.interval !== undefined ? opts.interval : 2000, //间隔值
            ...
            _Reverse = opts.reverse !== undefined ? opts.reverse : false; //进行值的附加的时候 是否相反
        var _TouchDistance;
        if (_IsVerticalTouch) {
        ...
        _Slider.on('start', function (e) {
            _Self.ds('start');
            _TouchStartDistance = sliderObject[valueName];
            if(_Reverse)_TouchStartDistance=-_TouchStartDistance;
