...
function SetWechatShare(title, content, url, imgUrl, shareSuccessCallBack, shareCancelCallBack, shareFailCallBack) {
    ...
    /// <param name="shareCancelCallBack" type="func">分享取消回调函数,参数为微信分享类型WechatShareType </param>
    /// <param name="shareFailCallBack" type="func">分享失败回调函数,参数为微信分享类型WechatShareType </param>
    //if (JsApiWXConfigItf == "") {
...
function SetWechatShareToFriend(title, content, url, imgUrl, shareSuccessCallBack, shareCancelCallBack, shareFailCallBack) {
    ...
    if (WechatShareFriendUrl.indexOf("http:") < 0) { //不是绝对路径
        WechatShareFriendUrl = GetAbsoluteUrl(WechatShareFriendUrl);
    }
    if (imgUrl) {
        WechatShareFriendImgUrl = imgUrl;
    } else {
    ...
    if (WechatShareFriendImgUrl.indexOf("http:") < 0) { //不是绝对路径
        WechatShareFriendImgUrl = GetAbsoluteUrl(WechatShareFriendImgUrl);
    }
...
function SetWechatShareToTimeline(title, content, url, imgUrl, shareSuccessCallBack, shareCancelCallBack, shareFailCallBack) {
    ...
    if (WechatShareTimelineUrl.indexOf("http:") < 0) { //不是绝对路径
        WechatShareTimelineUrl = GetAbsoluteUrl(WechatShareTimelineUrl);
    }
    if (imgUrl) {
        WechatShareTimelineImgUrl = imgUrl;
    } else {
    ...
    if (WechatShareTimelineImgUrl.indexOf("http:") < 0) { //不是绝对路径
        WechatShareTimelineImgUrl = GetAbsoluteUrl(WechatShareTimelineImgUrl);
    }
    WechatShareTimelineSuccessCallBack = shareSuccessCallBack;
    WechatShareTimelineCancelCallBack = shareCancelCallBack;
    WechatShareTimelineFailCallBack = shareFailCallBack;
