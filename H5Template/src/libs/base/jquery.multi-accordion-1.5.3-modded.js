...
(function($){
	...
	$.widget('ui.multiAccordion', {
		...
		_create: function() {
			...
			$h3.bind('click', function(e){
				...
				if ($this.hasClass(options._classes.stateDefault)) {
					self._showTab($this, true);
				} else {
					self._hideTab($this, true);
				}
		...
		_showTab: function($this, addToActive, animate) {
			...
			if(addToActive){
				var $h3 = this.element.children('h3');
				var elem = $this[0];
				var i = $h3.index(elem);
				if(!options.active || !(options.active instanceof Array)){
					options.active = [i];
				}else{
					var ind = options.active.indexOf(i);
					if(ind==-1)options.active.push(i);
				}
			}
		...
		_hideTab: function($this, removeFromActive, animate) {
			...
			if(removeFromActive){
				...
				if(options.active && (options.active instanceof Array)){
					var ind = options.active.indexOf(i);
					if(ind!=-1)options.active.splice(ind, 1);
				}
			}
		...
		showAll: function(){
			...
			//self._setActiveTabs(indices);
			self.option("active", indices);
		},
		hideAll: function(){
			var self = this;
			//self._setActiveTabs([]);
		...
		_dispatchChange : function(){
			if(this.options.change)this.options.change();
		},
